...
const isActiveModule = (moduleName) => {
  let activeModule = page.props?.activeModule
  if (activeModule === null && ['admin', 'user'].includes(moduleName)) return true

  if (page.props?.activeModule && moduleName) {
    return activeModule.toLowerCase() == moduleName.toLowerCase().replace('-', '')
  }
  return false
}
</script>
      ...
      <template v-for="groupeMenu in sidebarMenu" :key="groupeMenu.id">
        ...
        <li v-for="menu in groupeMenu.links" :key="menu.id">
          <template v-if="menu.children?.length > 0">
            <a href="javascript:void(0);" class="sidebar-menu" :class="{ active: isActive(menu) }">
            ...
            <ul class="sidebar-submenu">
              <template v-for="subItem in menu.children" :key="subItem.text">
                <li v-if="canAccess(subItem)">
                  <component
                    :is="isActiveModule(groupeMenu?.module) ? 'Link' : 'a'"
