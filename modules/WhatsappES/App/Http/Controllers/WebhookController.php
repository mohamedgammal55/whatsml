...
class WebhookController extends Controller
{
    ...
    public function verify(Request $request)
    {
        ...
        $webhook_verify_token = config('whatsappes.webhook_verify_token');

        if ($mode !== 'subscribe' || $webhook_verify_token != $verify_token) {
            return response([], 403);
        }
    ...
    private function handleEntries(array $entries)
    {
        foreach ($entries as $entryItem) {
            $platform = Platform::query()->where('uuid', $entryItem['id'])->first();
            if (!$platform) {
                continue;
            }
    ...
    private function handleNewMessages(array $value)
    {
        ...
        foreach ($value['messages'] ?? [] as $message) {
            ...
            if (in_array($messageType, ['document', 'sticker', 'image', 'video', 'audio'])) {
                $mimeType = $message[$messageType]['mimetype'] ?? null;
                $ext = $this->getExtension($mimeType);

                if (!$ext) {
                    $params['meta']['media_url'] = "";
                    $params['meta']['unsupported'] = true;
                } else {

                    $mediaId = $message[$messageType]['id'];
                    $accessToken = $conversation->platform->access_token;
                    $phoneNumberId = $conversation->platform->uuid;
                    $waClient = WhatsappClient::make(
                        $accessToken,
                        $phoneNumberId
                    );
                    $mediaInfoRes = $waClient->getMediaInfo($mediaId);

                    if ($mediaInfoRes->successful()) {
                        $fileUrl = $mediaInfoRes->json('url');
                        $fileRes = $waClient->getMedia($fileUrl);
                        if ($fileRes->successful()) {
                            $params['meta'] = $mediaInfoRes->json();

                            $uploadDir = "uploads/message/$messageType";

                            if (!File::isDirectory($uploadDir)) {
                                File::makeDirectory($uploadDir, 0777, true);
                            }

                            $fileName = $message[$messageType]['filename'] ?? $mediaId;
                            $uploadPath = "$uploadDir/$fileName.{$ext}";

                            Storage::put($uploadPath, $fileRes->getBody());

                            $params['meta']['media_url'] = Storage::url($uploadPath);
                        }
                    }
                }
            }
